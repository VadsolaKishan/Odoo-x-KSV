  ...
  return (
    <div className="space-y-6">
      ...
      <button
        ...
        className="flex items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-800 dark:hover:text-dark-200 transition-colors"
      >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-display font-extrabold text-slate-800 dark:text-dark-100 tracking-tight">
              {rfq.title}
            </h1>
            <StatusBadge status={rfq.status} />
          </div>
          <p className="text-[10px] text-slate-400 dark:text-dark-500 font-semibold mt-1">
      ...
      <div className="glass-panel p-4 rounded-2xl bg-white dark:bg-dark-900">
        <h3 className="text-xs font-bold text-slate-400 dark:text-dark-500 uppercase tracking-wider px-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-panel p-6 rounded-2xl bg-white dark:bg-dark-900 space-y-4">
            ...
            <p className="text-xs text-slate-650 dark:text-dark-350 leading-relaxed font-medium">
              {rfq.description}
            </p>

            <div className="pt-2">
              <p className="text-xs font-bold text-slate-800 dark:text-dark-200 mb-2.5">Catalog Items List</p>
              <div className="rounded-xl border border-slate-150 dark:border-dark-800 overflow-hidden">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="bg-slate-50/50 dark:bg-dark-950/40 text-[10px] font-bold text-slate-400 dark:text-dark-550 border-b border-slate-150 dark:border-dark-800 uppercase select-none">
                  ...
                  <tbody className="divide-y divide-slate-150 dark:divide-dark-800 text-slate-700 dark:text-dark-300">
                    {rfq.items.map((item, index) => (
                      <tr key={index} className="hover:bg-slate-50/30 dark:hover:bg-dark-850/10">
          ...
          <div className="glass-panel p-6 rounded-2xl bg-white dark:bg-dark-900 space-y-4">
            ...
            {rfqQuotes.length === 0 ? (
              <p className="text-xs text-slate-400 dark:text-dark-500 text-center py-6">
                Waiting for supplier quotations to be submitted.
              </p>
            ) : (
              <div className="divide-y divide-slate-150 dark:divide-dark-800/60">
                {rfqQuotes.map((quote) => {
                  ...
                  return (
                    <div key={quote.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-slate-800 dark:text-dark-200">{quote.vendorName}</span>
                          {isChosen && (
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-dark-500">
                          Total Bid: <span className="font-bold text-slate-700 dark:text-dark-300">{formatIndianCurrency(quote.totalCost)}</span> • Delivery: <span className="font-bold text-slate-700 dark:text-dark-300">{quote.deliveryTimeDays} days</span>
                        </p>
                        <p className="text-[10px] text-slate-500 dark:text-dark-400 italic">"{quote.remarks}"</p>
                      </div>
                      ...
                      {rfq.status === 'Quotes Gathered' && (
                        <button
                          ...
                          className="px-3 py-1.5 text-[10px] font-bold bg-white dark:bg-dark-900 text-slate-700 dark:text-dark-300 border border-slate-200 dark:border-dark-800 rounded-lg hover:border-brand-500 hover:text-brand-600 dark:hover:text-brand-450 shadow-sm active:scale-95 transition-all whitespace-nowrap"
                        >
                          Select Quote
                        </button>
                      )}
        ...
        <div className="space-y-6">
          <div className="glass-panel p-6 rounded-2xl bg-white dark:bg-dark-900 space-y-4">
            ...
            <div className="space-y-3">
              {invitedVendors.map((vendor) => (
                <div key={vendor.id} className="p-3 border border-slate-150 dark:border-dark-800 rounded-xl flex items-center justify-between">
                  <div className="min-w-0 pr-1">
                    <p className="text-xs font-bold text-slate-800 dark:text-dark-200 truncate">{vendor.name}</p>
                    <p className="text-[10px] text-slate-400 dark:text-dark-500 truncate">{vendor.email}</p>
                  </div>
                  <span className="text-[10px] font-semibold text-slate-500 dark:text-dark-450 shrink-0">
          ...
          {/* Quick info sheet */}
          <div className="glass-panel p-6 rounded-2xl bg-white dark:bg-dark-900 space-y-3 text-xs leading-normal">
            <h4 className="font-bold text-slate-800 dark:text-dark-200">Bidding Milestones</h4>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-slate-450">Bidding Opens:</span>
                <span className="font-medium text-slate-700 dark:text-dark-300">{rfq.createdAt}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-450">Closing Date:</span>
                <span className="font-medium text-slate-700 dark:text-dark-300 flex items-center gap-1">
                  <Calendar className="w-3.5 h-3.5 text-danger-550" /> {rfq.deadline}
                </span>
              </div>
              <div className="flex justify-between items-center border-t border-slate-100 dark:border-dark-800 pt-2">
                <span className="text-slate-450">PO Link Code:</span>
                <span className="font-bold text-slate-800 dark:text-dark-200">
                  {rfq.poId ? (
