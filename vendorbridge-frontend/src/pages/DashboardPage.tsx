...
export default function DashboardPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (norm === 'completed' || norm === 'approved') {
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/10 text-brand-green border border-brand-green/20">
          Approved
        </span>
      );
      ...
      return (
        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-500/10 text-gray-400 border border-white/5">
          Draft
        </span>
      );
    }
    ...
    <MainLayout>
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white">
            Welcome back, {user ? `${user.first_name}` : 'Procurement Officer'}
          </h1>
          <p className="text-text-secondary text-sm mt-1">Here is a summary of your procurement pipelines today.</p>
        </div>
      ...
      {loadingSummary ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          >
            <div>
              <span className="text-2xl font-bold text-white tracking-tight">{activeRFQsCount}</span>
          ...
          >
            <div>
              <span className="text-2xl font-bold text-white tracking-tight">{pendingApprovalsCount}</span>
              <span className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mt-1">
                Pending Approvals
              </span>
            </div>
          ...
          >
            <div>
              <span className="text-2xl font-bold text-white tracking-tight">{totalSpendFormatted}</span>
              <span className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mt-1">
                PO's this month
              </span>
            </div>
          ...
          >
            <div>
              <span className="text-2xl font-bold text-white tracking-tight">{overdueInvoicesCount}</span>
              <span className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mt-1">
                overdue invoices
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
        ...
        <div className="lg:col-span-7 glass-card rounded-xl p-6 flex flex-col justify-between border border-white/5">
          <div>
                  ...
                  <tbody className="divide-y divide-subtle/50">
                    {purchaseOrders.map((po: any) => (
                      <tr 
                        ...
                        onClick={() => navigate(`/purchase-orders`)}
                      >
                        <td className="py-3.5 px-4 font-semibold text-white group-hover:text-brand-green transition-colors">
                          {po.po_number}
                        </td>
